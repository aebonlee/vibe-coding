...
const Register = (): ReactElement => {
  ...
  const handleGoogle = async () => {
    try { await signInWithGoogle(); } catch (err) { showToast((err as Error).message, 'error'); }
  };

  const handleKakao = async () => {
    try { await signInWithKakao(); } catch (err) { showToast((err as Error).message, 'error'); }
  };
    ...
    <div className="auth-container">
      <div className="auth-card">
        ...
        <div className="social-login-buttons">
          <button className="social-btn google" onClick={handleGoogle} type="button">
          ...
          <button className="social-btn kakao" onClick={handleKakao} type="button">
            <svg viewBox="0 0 24 24" width="20" height="20"><path d="M12 3C6.48 3 2 6.36 2 10.5c0 2.67 1.77 5.02 4.44 6.35-.13.47-.84 3.01-.87 3.21 0 0-.02.08.04.11.06.04.13.01.13.01 .17-.02 2.01-1.31 2.86-1.95.79.12 1.6.18 2.4.18 5.52 0 10-3.36 10-7.5S17.52 3 12 3z" fill="#3C1E1E"/></svg>
            {t('auth.kakaoLogin')}
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div className="form-group">
            ...
            <input type="password" value={passwordConfirm} onChange={e => setPasswordConfirm(e.target.value)} placeholder={t('auth.passwordConfirmPlaceholder')} required />
          </div>
