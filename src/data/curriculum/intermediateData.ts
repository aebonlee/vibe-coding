...
export const intermediateTopics: CurriculumTopic[] = [
  // ─── 1. 컨텍스트 관리 ───
  {
    id: 'int-1',
    slug: 'context-management',
    title: '컨텍스트 관리',
    description: 'AI에게 프로젝트 맥락을 효과적으로 전달하고, 대화 품질을 유지하는 기법을 학습합니다.',
    level: 'intermediate',
    order: 1,
    contents: [
      {
        id: 'int-1-1',
        title: 'CLAUDE.md와 Rules 파일 작성',
        description: 'AI 코딩 도구가 프로젝트를 이해하도록 컨텍스트 문서를 작성합니다.',
        content: `AI 코딩 도구는 매 대화마다 프로젝트의 맥락을 새로 파악해야 합니다. CLAUDE.md나 .cursorrules 같은 컨텍스트 파일을 작성하면 AI가 프로젝트의 기술 스택, 코딩 컨벤션, 아키텍처를 즉시 이해할 수 있습니다.
...
좋은 컨텍스트 문서는 새로운 팀원에게 프로젝트를 설명하는 온보딩 문서와 같은 역할을 합니다. AI도 사람과 마찬가지로 맥락을 충분히 이해할수록 더 정확한 코드를 생성합니다.`,
        keyPoints: [
          'CLAUDE.md는 프로젝트 루트에 위치하며 AI가 자동으로 읽는다',
          '기술 스택, 컨벤션, 아키텍처를 명시적으로 기술한다',
          'Anti-pattern을 명시하면 AI의 실수를 사전에 방지할 수 있다',
          '팀 프로젝트에서는 모든 팀원이 동일한 AI 컨텍스트를 공유한다'
        ],
        codeExamples: [
          {
            language: 'markdown',
            title: 'CLAUDE.md 예시',
            code: `# Project: My SaaS App
...
- CSS-in-JS 사용 금지 — TailwindCSS만 사용`
      ...
      },
      {
        id: 'int-1-2',
        title: '프로젝트 구조 설명 기법',
        description: '복잡한 프로젝트 구조를 AI에게 효과적으로 전달하는 방법을 배웁니다.',
        content: `AI에게 프로젝트 구조를 설명할 때는 단순히 파일 목록을 나열하는 것이 아니라, 각 디렉토리와 파일의 역할과 관계를 명확히 전달해야 합니다.
...
특히 대규모 프로젝트에서는 현재 작업 중인 영역에 집중하여 설명하는 것이 중요합니다. AI의 컨텍스트 윈도우는 유한하므로, 관련 없는 파일 정보는 노이즈가 됩니다.`,
        keyPoints: [
          '트리 구조로 디렉토리 레이아웃을 시각적으로 전달한다',
          '현재 작업 영역에 해당하는 파일만 선별적으로 공유한다',
          '파일 간 import 관계를 설명하면 AI가 수정 범위를 정확히 파악한다',
          '설정 파일(tsconfig, vite.config 등)은 빌드 이슈 해결 시 반드시 공유한다'
        ],
        codeExamples: [
          {
            language: 'markdown',
            title: '프로젝트 구조 설명 프롬프트 예시',
            code: `## 현재 작업 중인 영역
...
4. 미인증 시 /login으로 리다이렉트`
          }
        ]
      },
      {
        id: 'int-1-3',
        title: '대화 길이 관리와 컨텍스트 윈도우',
        description: 'AI와의 대화가 길어질 때 품질을 유지하는 전략을 학습합니다.',
        content: `AI 코딩 도구의 대화가 길어지면 초기 맥락이 희석되어 코드 품질이 저하될 수 있습니다. 이를 "컨텍스트 드리프트(Context Drift)"라고 합니다.
...
Claude Code에서는 /compact 명령으로 대화를 요약하여 컨텍스트를 절약할 수 있습니다. Cursor에서는 새 Composer 세션을 시작할 때 이전 대화의 핵심 결정사항을 첫 메시지에 포함합니다.`,
        keyPoints: [
          '하나의 대화에서 하나의 기능을 완성하는 것이 최적이다',
          '20회 이상 대화가 이어지면 새 세션 시작을 고려한다',
          '/compact(Claude Code)로 대화를 요약하여 토큰을 절약할 수 있다',
          '새 대화를 시작할 때 이전 결정사항 요약을 첫 메시지로 전달한다'
        ],
        codeExamples: [
          {
            language: 'markdown',
            title: '새 대화 시작 시 컨텍스트 전달 예시',
            code: `## 이전 대화 요약
...
- RLS: 본인 아바타만 CRUD 가능 (정책 설정 완료)`
          }
        ]
      },
      {
        id: 'int-1-4',
        title: '멀티 프로젝트 컨텍스트 관리',
        description: '여러 프로젝트를 동시에 관리할 때 컨텍스트를 효율적으로 전환하는 방법을 배웁니다.',
        content: `실무에서는 여러 프로젝트를 동시에 진행하는 경우가 많습니다. 각 프로젝트의 컨텍스트를 효율적으로 관리하는 것이 생산성의 핵심입니다.
...
특히 동일한 기술 스택(예: React + Supabase)을 사용하는 프로젝트들은 공통 패턴을 템플릿화하면 AI가 일관된 코드를 생성하는 데 도움이 됩니다.`,
        keyPoints: [
          '프로젝트별 CLAUDE.md로 독립적인 컨텍스트를 유지한다',
          '글로벌 설정과 프로젝트 설정을 계층적으로 관리한다',
          '공통 패턴은 템플릿으로 만들어 재사용한다',
          'DB 프리픽스 등 프로젝트 고유 설정을 명시하여 혼동을 방지한다'
        ],
        codeExamples: [
          {
            language: 'yaml',
            title: '글로벌 설정 예시 (~/.claude/CLAUDE.md)',
            code: `# Global Preferences
...
- barrel exports (index.ts) 사용`
  ...
  // ─── 2. 프롬프트 심화 기법 ───
  {
    ...
    title: '프롬프트 심화 기법',
    description: '역할 부여, 제약 조건 설정, 체인 오브 쏘트 등 고급 프롬프트 엔지니어링을 학습합니다.',
    level: 'intermediate',
    order: 2,
    contents: [
      {
        id: 'int-2-1',
        title: '역할 부여와 페르소나 설정',
        description: 'AI에게 전문가 역할을 부여하여 코드 품질을 높이는 기법을 배웁니다.',
        content: `AI에게 특정 전문가 역할을 부여하면 해당 분야의 모범 사례와 패턴을 반영한 코드를 생성합니다. 이를 "페르소나 프롬프팅"이라고 합니다.
...
역할 부여는 CLAUDE.md의 첫 줄에 배치하거나, 특정 작업에 대한 프롬프트 시작 부분에 명시합니다. 동일한 프로젝트에서도 작업에 따라 역할을 전환하는 것이 효과적입니다.`,
        keyPoints: [
          '역할을 부여하면 AI가 해당 분야의 모범 사례를 적용한다',
          '코드 리뷰 시에는 "시니어 리뷰어" 역할이 더 엄격한 피드백을 제공한다',
          '보안, 성능, 접근성 등 특정 관점을 지정할 수 있다',
          '역할은 CLAUDE.md에 상시 설정하거나 필요 시에만 부여할 수 있다'
        ],
        codeExamples: [
          {
            language: 'markdown',
            title: '역할 부여 프롬프트 예시',
            code: `# 역할 부여 예시 1: 아키텍처 설계
...
useCallback을 적절히 적용해주세요.`
          }
        ]
      },
      {
        id: 'int-2-2',
        title: '제약 조건과 출력 형식 지정',
        description: 'AI의 출력을 원하는 형태로 제어하는 제약 조건 설정 기법을 배웁니다.',
        content: `제약 조건을 명확하게 설정하면 AI가 불필요한 코드를 생성하지 않고, 프로젝트의 규칙에 맞는 결과물을 제공합니다.
...
제약 조건이 명확할수록 AI의 출력이 예측 가능해지고, 수정 작업이 줄어듭니다. "하지 말아야 할 것"을 명시하는 것이 "해야 할 것"만큼 중요합니다.`,
        keyPoints: [
          '"~하지 마세요" 형태의 부정 제약이 품질 향상에 매우 효과적이다',
          '출력 형식을 지정하면 후처리 작업이 줄어든다',
          '기존 코드의 패턴을 예시로 보여주면 일관성이 유지된다',
          '제약 조건은 5개 이내로 핵심만 전달하는 것이 효과적이다'
        ],
        codeExamples: [
          {
            language: 'markdown',
...
2. 그 다음 컴포넌트 코드
3. 마지막으로 사용법 예시
4. 각 코드 블록에 파일 경로 주석 포함`
          }
        ]
      },
      {
        id: 'int-2-3',
        title: '체인 오브 쏘트와 단계적 사고',
        description: 'AI에게 단계적으로 생각하도록 유도하여 복잡한 문제를 해결하는 기법을 배웁니다.',
        content: `체인 오브 쏘트(Chain of Thought, CoT)는 AI에게 "단계별로 생각해보세요"라고 요청하여 복잡한 문제의 해결 품질을 높이는 기법입니다.
...
**Few-shot 예시와 결합하면 더욱 강력합니다.** 기대하는 사고 과정의 예시를 1-2개 보여주면 AI가 동일한 패턴으로 추론합니다.`,
        keyPoints: [
          '"단계별로 분석해주세요"라는 지시가 CoT를 활성화한다',
          '복잡한 로직일수록 CoT의 품질 향상 효과가 크다',
          'Few-shot 예시와 결합하면 일관된 추론 패턴을 유도할 수 있다',
          '단순 코드 생성에는 CoT가 불필요하며 오히려 비효율적이다'
        ],
        codeExamples: [
          {
            language: 'markdown',
            title: '체인 오브 쏘트 프롬프트 예시',
            code: `## 요청
...
5. 테스트 케이스 작성`
          ...
          {
            language: 'typescript',
            title: 'Few-shot 예시를 포함한 프롬프트 결과',
            code: `// Step 1: 할인 규칙 우선순위 정의
// 1) VIP 할인 (기본)
// 2) 금액 기반 할인 (추가)
// 3) 최대 할인율 캡 적용
// 4) 쿠폰 차감 (할인율 캡과 별도)
...
export function calculateDiscount(input: DiscountInput): DiscountResult {
  ...
  return { finalPrice, discountRate, discountAmount, couponApplied, breakdown };
      ...
      },
      {
        id: 'int-2-4',
        title: '복잡한 요구사항 분해',
        description: '대규모 기능 요구사항을 AI가 처리하기 좋은 단위로 분해하는 전략을 배웁니다.',
        content: `하나의 프롬프트에 너무 많은 요구사항을 넣으면 AI의 출력 품질이 떨어집니다. 복잡한 기능을 적절한 단위로 분해하여 순차적으로 요청하는 것이 핵심입니다.
...
예를 들어 "게시판 기능을 만들어주세요"라는 큰 요청 대신, 타입 정의 → DB 스키마 → CRUD 훅 → 목록 페이지 → 상세 페이지 → 작성 폼 순서로 분해합니다.`,
        keyPoints: [
          '하나의 프롬프트에 파일 3개 이상 생성을 요청하면 품질이 저하된다',
          '타입/인터페이스를 먼저 합의한 후 구현을 요청하는 것이 효과적이다',
          '의존 관계가 있는 코드는 반드시 순서대로 요청한다',
          '각 단계의 결과를 검증한 후 다음 단계로 진행한다'
        ],
        codeExamples: [
          {
            language: 'markdown',
            title: '게시판 기능 분해 예시 (5단계)',
            code: `## Step 1: 타입 정의 (첫 번째 프롬프트)
...
PostInput 타입을 사용합니다."`
  ...
  {
    id: 'int-3',
    slug: 'ai-ide-advanced',
    title: 'AI IDE 고급 활용',
    description: 'Cursor Rules, Composer, 멀티파일 편집, Tab 자동완성 등 AI IDE의 고급 기능을 활용합니다.',
    level: 'intermediate',
    order: 3,
    contents: [
      {
        id: 'int-3-1',
        title: '.cursorrules 파일 작성',
        description: 'Cursor IDE의 프로젝트별 규칙 파일을 작성하여 AI 동작을 커스터마이징합니다.',
        content: `.cursorrules 파일은 Cursor IDE가 코드를 생성할 때 따르는 프로젝트별 규칙을 정의합니다. 프로젝트 루트에 위치하며, 모든 AI 기능(Tab 완성, Cmd+K, Chat, Composer)에 적용됩니다.
...
.cursorrules는 CLAUDE.md와 비슷한 역할을 하지만, Cursor IDE에 특화된 형식입니다. 프로젝트의 기술 스택, 코딩 컨벤션, 자주 사용하는 패턴을 명시하면 AI가 프로젝트에 맞는 코드를 생성합니다.`,
        keyPoints: [
          '.cursorrules는 프로젝트 루트에 위치하며 자동으로 적용된다',
          'Tab 완성, Chat, Composer 등 모든 AI 기능에 영향을 준다',
          '규칙이 너무 길면 오히려 역효과 — 핵심 규칙만 포함한다',
          '팀원이 공유하도록 Git에 커밋하는 것을 권장한다'
        ],
        codeExamples: [
          {
            language: 'markdown',
            title: '.cursorrules 파일 예시',
            code: `You are an expert React + TypeScript developer.
...
- No console.log in production code`
      ...
      {
        id: 'int-3-2',
        title: 'Cursor Composer 활용',
        description: 'Composer를 사용하여 여러 파일을 동시에 생성하고 수정하는 방법을 배웁니다.',
        content: `Cursor Composer는 여러 파일을 동시에 생성하거나 수정할 수 있는 강력한 기능입니다. 단일 파일 편집인 Cmd+K와 달리, Composer는 프로젝트 전체를 조망하며 작업할 수 있습니다.
...
4. 너무 많은 파일(10개 이상)을 한 번에 변경하지 않습니다`,
        keyPoints: [
          'Composer는 여러 파일을 동시에 수정할 수 있어 일관성을 유지한다',
          '@file 참조로 AI에게 정확한 파일 컨텍스트를 제공한다',
          '변경사항을 적용하기 전에 반드시 diff를 검토한다',
          '한 번에 5-7개 파일 이내로 제한하는 것이 안전하다'
        ],
        codeExamples: [
          {
            language: 'markdown',
            title: 'Composer에 효과적인 프롬프트 예시',
            code: `## Composer 프롬프트: 새 기능 추가
...
- 중복 좋아요 방지 로직 포함`
      ...
      },
      {
        id: 'int-3-3',
        title: 'Tab 자동완성 최적화',
        description: 'Cursor의 Tab 자동완성 기능을 최대한 활용하여 코딩 속도를 높이는 방법을 배웁니다.',
        content: `Cursor의 Tab 자동완성(Copilot++)은 단순한 코드 완성을 넘어, 주변 코드의 패턴을 학습하여 다음에 작성할 코드를 예측합니다. 이 기능을 극대화하려면 AI가 패턴을 잘 인식할 수 있도록 코드를 작성해야 합니다.
...
- 유사한 구조의 컴포넌트
- 테스트 케이스 작성 (하나의 테스트 후 나머지를 예측)`,
        keyPoints: [
          '일관된 코딩 패턴이 Tab 자동완성의 정확도를 높인다',
          '주석을 먼저 작성하면 AI가 구현 의도를 파악하여 더 나은 완성을 제공한다',
          '타입 정의 후 구현하면 타입에 맞는 코드가 자동완성된다',
          'Tab을 수락하기 전에 제안된 코드를 반드시 검토한다'
        ],
        codeExamples: [
          {
            language: 'typescript',
            title: 'Tab 완성을 유도하는 코딩 패턴',
            code: `// 1. 타입을 먼저 정의하면 Tab이 구현을 정확히 예측
...
async function getAllUsers(): Promise<User[]> {
  ...
  return data ?? [];
      ...
      },
      {
        id: 'int-3-4',
        title: '멀티파일 편집 워크플로우',
        description: '여러 파일에 걸친 변경을 효율적으로 수행하는 AI IDE 워크플로우를 배웁니다.',
        content: `실제 개발에서는 하나의 기능이 여러 파일에 걸쳐 있습니다. AI IDE를 활용한 멀티파일 편집 워크플로우를 체계화하면 생산성이 크게 향상됩니다.
...
Cursor의 경우 Composer로 한 번에 처리할 수 있지만, Claude Code에서는 자동으로 멀티파일 편집을 수행합니다. 핵심은 변경 순서와 범위를 명확히 지시하는 것입니다.`,
        keyPoints: [
          '타입 정의 파일부터 수정하고, 의존하는 파일을 순차적으로 수정한다',
          'AI에게 변경이 필요한 파일 목록을 먼저 확인 요청하면 누락을 방지한다',
          '각 파일 수정 후 tsc 타입 체크로 오류를 즉시 발견한다',
          '너무 많은 파일을 한 번에 변경하면 롤백이 어려워진다'
        ],
        codeExamples: [
          {
            language: 'bash',
            title: 'Claude Code에서 멀티파일 편집 프롬프트',
            code: `# Claude Code에서의 멀티파일 편집 프롬프트 예시
...
> "tsc --noEmit으로 타입 오류를 확인해주세요"`
  ...
  // ─── 4. 풀스택 개발 워크플로우 ───
  {
    ...
    title: '풀스택 개발 워크플로우',
    description: 'React + Supabase 조합으로 풀스택 앱을 AI와 함께 구축하는 워크플로우를 학습합니다.',
    level: 'intermediate',
    order: 4,
    contents: [
      {
        id: 'int-4-1',
        title: 'DB 스키마 설계와 AI',
        description: 'AI와 함께 Supabase 데이터베이스 스키마를 설계하고 생성하는 방법을 배웁니다.',
        content: `풀스택 개발의 첫 단계는 데이터베이스 스키마 설계입니다. AI에게 비즈니스 요구사항을 전달하면 정규화된 스키마를 설계하고, Supabase SQL을 생성할 수 있습니다.
...
테이블 프리픽스를 사용하면 여러 프로젝트가 하나의 Supabase 인스턴스를 공유할 때 충돌을 방지할 수 있습니다. 예를 들어 게시판 기능의 테이블은 board_posts, board_comments와 같이 프리픽스를 붙입니다.`,
        keyPoints: [
          'AI에게 비즈니스 요구사항을 자연어로 전달하면 스키마를 설계해준다',
          'RLS(Row Level Security) 정책은 스키마 생성 시 함께 설정한다',
          '테이블 프리픽스로 멀티 프로젝트 환경에서 충돌을 방지한다',
          'created_at, updated_at 컬럼은 모든 테이블에 기본 포함한다'
        ],
        codeExamples: [
          {
            language: 'typescript',
            title: 'AI에게 스키마 설계 요청 및 결과',
            code: `-- AI에게 전달한 요구사항:
-- "온라인 서점의 DB를 설계해주세요.
...
CREATE POLICY "본인 리뷰만 수정/삭제"
  ...
  USING (auth.uid() = user_id);`
      ...
      {
        id: 'int-4-2',
        title: 'Supabase Auth 구현',
        description: 'AI와 함께 Supabase 인증 시스템을 구현하는 방법을 배웁니다.',
        content: `Supabase Auth는 이메일/비밀번호, OAuth(Google, GitHub 등), 매직 링크 등 다양한 인증 방식을 제공합니다. AI에게 인증 요구사항을 전달하면 AuthContext, 로그인/회원가입 페이지, 보호된 라우트를 생성할 수 있습니다.
...
특히 AuthContext는 앱 전체에서 사용되므로, 처음부터 견고하게 설계하는 것이 중요합니다.`,
        keyPoints: [
          'AuthContext는 앱 전체의 인증 상태를 관리하는 핵심 컴포넌트이다',
          'onAuthStateChange로 세션 변화를 실시간 감지한다',
          '보호된 라우트는 ProtectedRoute 컴포넌트로 구현한다',
          'user_profiles 테이블에 추가 사용자 정보를 저장한다'
        ],
        codeExamples: [
          {
            language: 'typescript',
            title: 'AI로 생성한 AuthContext 핵심 구조',
...
export const useAuth = () => {
  ...
  return ctx;
      ...
      {
        id: 'int-4-3',
        title: 'CRUD 개발과 커스텀 훅',
        description: 'AI와 함께 재사용 가능한 CRUD 커스텀 훅을 만드는 패턴을 학습합니다.',
        content: `CRUD(Create, Read, Update, Delete)는 대부분의 웹 애플리케이션의 핵심입니다. AI와 함께 재사용 가능한 커스텀 훅으로 CRUD를 구현하면 일관된 패턴으로 빠르게 개발할 수 있습니다.
...
AI에게 첫 번째 훅(예: useProducts)을 만들게 한 후, 동일한 패턴으로 나머지 훅을 생성하면 프로젝트 전체의 일관성을 유지할 수 있습니다.`,
        keyPoints: [
          'CRUD 훅은 데이터, 로딩, 에러 상태를 함께 반환한다',
          '첫 번째 훅의 패턴을 기준으로 나머지를 일관되게 생성한다',
          '낙관적 업데이트로 사용자 경험을 개선한다',
          'AI에게 기존 훅의 패턴을 보여주면 동일한 스타일로 생성한다'
        ],
        codeExamples: [
          {
            language: 'typescript',
            title: '재사용 가능한 CRUD 훅 패턴',
            code: `// src/hooks/useProducts.ts
  ...
  return {
    ...
    createProduct, updateProduct, deleteProduct,
      ...
      },
      {
        id: 'int-4-4',
        title: 'RLS 정책 설계',
        description: 'Supabase Row Level Security 정책을 AI와 함께 설계하여 데이터를 안전하게 보호합니다.',
        content: `RLS(Row Level Security)는 Supabase의 핵심 보안 기능으로, 데이터베이스 레벨에서 행별 접근 권한을 제어합니다. AI에게 접근 규칙을 자연어로 설명하면 적절한 RLS 정책을 생성할 수 있습니다.
...
4. 조건부 공개 (is_public이 true인 것만 공개)`,
        keyPoints: [
          'RLS는 프론트엔드 검증보다 안전한 서버 레벨 접근 제어이다',
          'auth.uid()로 현재 사용자를 식별한다',
          '정책이 없으면 해당 테이블에 아무도 접근할 수 없다 (기본 거부)',
          'superadmin은 별도 정책으로 모든 접근을 허용한다'
        ],
        codeExamples: [
          {
            language: 'typescript',
            title: 'AI에게 RLS 요구사항 전달 예시와 결과',
            code: `-- AI에게 전달한 요구사항:
-- "게시판 RLS를 만들어주세요.
...
CREATE POLICY "본인 댓글 삭제"
  ...
  // ─── 5. Skills & Commands 활용 ───
  {
    id: 'int-5',
    slug: 'skills-commands',
    title: 'Skills & Commands 활용',
    description: 'Claude Code와 Cursor의 커스텀 명령어 및 프로젝트별 설정을 활용합니다.',
    level: 'intermediate',
    order: 5,
    contents: [
      {
        id: 'int-5-1',
        title: 'Claude Code 슬래시 명령어',
        description: 'Claude Code의 내장 명령어와 커스텀 슬래시 명령어를 활용하는 방법을 배웁니다.',
        content: `Claude Code는 다양한 슬래시 명령어를 제공하여 반복적인 작업을 효율화합니다. 내장 명령어 외에도 프로젝트별 커스텀 명령어를 만들어 팀 워크플로우를 표준화할 수 있습니다.
...
커스텀 명령어는 반복적인 작업(배포, 테스트, 린트 수정 등)을 자동화하는 데 매우 유용합니다.`,
        keyPoints: [
          '/compact로 긴 대화의 토큰을 절약할 수 있다',
          '.claude/commands/ 디렉토리에 커스텀 명령어를 정의한다',
          '커스텀 명령어는 /project: 접두사로 호출한다',
          '팀 공통 명령어는 Git에 커밋하여 공유한다'
        ],
        codeExamples: [
          {
            language: 'markdown',
            title: '.claude/commands/deploy.md (커스텀 배포 명령어)',
...
- CNAME의 도메인은 {프로젝트명}.dreamitbiz.com 형식`
          },
          {
            language: 'markdown',
            title: '.claude/commands/new-page.md (새 페이지 생성)',
...
- App.tsx의 라우트에 자동 추가`
      ...
      {
        id: 'int-5-2',
        title: 'Cursor @commands 활용',
        description: 'Cursor IDE의 @ 명령어를 활용하여 AI에게 정확한 컨텍스트를 제공합니다.',
        content: `Cursor의 @ 명령어는 AI에게 특정 컨텍스트를 제공하는 강력한 메커니즘입니다. 파일, 심볼, 문서 등을 직접 참조하여 AI가 정확한 정보를 기반으로 응답하도록 합니다.
...
@ 명령어를 효과적으로 조합하면 AI가 프로젝트의 정확한 맥락을 이해하고, 기존 코드와 일관된 결과물을 생성합니다.`,
        keyPoints: [
          '@file로 관련 파일을 명시하면 AI가 기존 패턴을 파악한다',
          '@codebase는 전체 코드에서 관련 부분을 자동으로 찾아준다',
          '@docs로 외부 라이브러리 문서를 참조할 수 있다',
          '여러 @ 명령어를 조합하면 매우 정확한 컨텍스트를 전달할 수 있다'
        ],
        codeExamples: [
          {
            language: 'markdown',
            title: 'Cursor @ 명령어 활용 예시',
            code: `## 예시 1: 기존 패턴에 맞게 새 훅 생성
...
에러 처리가 누락된 부분을 알려주세요"`
      ...
      },
      {
        ...
        description: 'AI 코딩 도구의 시스템 레벨 설정으로 일관된 동작을 보장합니다.',
        content: `Custom Instructions는 AI 코딩 도구에게 영구적으로 적용되는 지시사항입니다. 매번 프롬프트에 포함하지 않아도 항상 적용되므로, 개인적인 코딩 선호도를 설정하는 데 적합합니다.
...
**우선순위**: 프로젝트 설정이 글로벌 설정보다 우선합니다. 충돌이 있으면 더 구체적인 설정이 적용됩니다.`,
        keyPoints: [
          '글로벌 설정은 모든 프로젝트에 적용되는 공통 선호도를 설정한다',
          '프로젝트 설정은 해당 프로젝트만의 고유한 규칙을 설정한다',
          '프로젝트 설정이 글로벌 설정보다 우선한다',
          '설정 파일은 Git에 커밋하여 팀원과 공유할 수 있다'
        ],
        codeExamples: [
          {
            language: 'markdown',
            title: '계층적 설정 구조 예시',
            code: `# ~/.claude/CLAUDE.md (글로벌 — 모든 프로젝트)
## 응답 언어
- 코드 주석과 설명은 한국어로 작성
- 변수명과 함수명은 영어로 작성
...
- CSS 모듈 사용 금지 (TailwindCSS만)`
  ...
  // ─── 6. AI와 코드 리뷰 ───
  {
    ...
    slug: 'code-review',
    title: 'AI와 코드 리뷰',
    description: 'AI를 활용하여 코드 품질 개선, 리팩토링, 보안 취약점 검토, 성능 최적화를 수행합니다.',
    level: 'intermediate',
    order: 6,
    contents: [
      {
        id: 'int-6-1',
        title: 'AI 코드 리뷰 요청 기법',
        description: 'AI에게 효과적으로 코드 리뷰를 요청하는 방법과 관점 지정 기법을 배웁니다.',
        content: `AI 코드 리뷰는 사람 리뷰어를 대체하는 것이 아니라, 리뷰 품질을 높이는 보조 도구입니다. AI에게 구체적인 리뷰 관점을 지정하면 더 유용한 피드백을 받을 수 있습니다.
...
리뷰를 요청할 때 어떤 관점에 집중할지 명시하면, AI가 해당 영역에 대해 더 깊이 있는 피드백을 제공합니다. "전체적으로 리뷰해주세요"보다 "보안 관점에서 리뷰해주세요"가 훨씬 유용합니다.`,
        keyPoints: [
          '리뷰 관점을 명시하면 더 깊이 있는 피드백을 받는다',
          'Claude Code에서 /review 명령으로 코드 리뷰를 요청할 수 있다',
          'AI 리뷰는 사람 리뷰 전 사전 점검으로 활용하면 효과적이다',
          '리뷰 결과를 무조건 수용하지 말고 비판적으로 판단한다'
        ],
        codeExamples: [
          {
            language: 'markdown',
            title: '관점별 코드 리뷰 프롬프트',
            code: `## 보안 관점 리뷰 요청
...
- 하드코딩된 값이 있는가? (상수로 추출 필요)`
      ...
      },
      {
        id: 'int-6-2',
        title: '리팩토링 요청 패턴',
        description: 'AI에게 코드 리팩토링을 효과적으로 요청하는 패턴과 전략을 배웁니다.',
        content: `리팩토링은 기능은 동일하게 유지하면서 코드의 구조를 개선하는 작업입니다. AI에게 리팩토링을 요청할 때는 "무엇을 개선하고 싶은지"를 명확히 전달해야 합니다.
...
- 테스트가 있다면 리팩토링 후 테스트를 실행합니다`,
        keyPoints: [
          '리팩토링은 한 번에 하나의 변경만 수행하는 것이 안전하다',
          '"왜 리팩토링해야 하는지" 이유를 설명하면 더 좋은 결과를 얻는다',
          'AI가 제안한 리팩토링이 기존 동작을 변경하지 않는지 반드시 확인한다',
          'Git 커밋 후 리팩토링하여 언제든 되돌릴 수 있게 한다'
        ],
        codeExamples: [
          {
            language: 'typescript',
            title: '리팩토링 전후 비교 예시',
            code: `// ── 리팩토링 전: 하나의 거대한 함수 ──
  ...
  try {
    ...
    setError(err instanceof Error ? err.message : '등록 실패');
      ...
      },
      {
        id: 'int-6-3',
        title: '보안 취약점 검토',
        description: 'AI를 활용하여 웹 애플리케이션의 보안 취약점을 탐지하고 수정합니다.',
        content: `웹 애플리케이션의 보안은 개발 초기부터 고려해야 합니다. AI에게 코드의 보안 취약점을 검토 요청하면, 놓치기 쉬운 잠재적 위협을 발견할 수 있습니다.
...
- .env 파일이 Git에 커밋되지 않도록 .gitignore 설정`,
        keyPoints: [
          'dangerouslySetInnerHTML 사용 시 DOMPurify로 반드시 sanitize한다',
          'API 키와 시크릿은 환경 변수로 관리하고 .env를 gitignore한다',
          'Supabase anon 키만 프론트엔드에서 사용 가능하다',
          'AI에게 OWASP Top 10 관점의 리뷰를 요청하면 체계적인 검토가 가능하다'
        ],
        codeExamples: [
          {
            language: 'typescript',
  ...
  const { data } = await supabase
    ...
    .textSearch('title', sanitizedQuery);
      ...
      },
      {
        id: 'int-6-4',
        title: '성능 최적화 제안 받기',
        description: 'AI에게 React 애플리케이션의 성능 최적화 방안을 제안받는 방법을 배웁니다.',
        content: `React 애플리케이션의 성능 문제는 주로 불필요한 리렌더링, 무거운 연산, 큰 번들 사이즈에서 발생합니다. AI에게 성능 관점의 리뷰를 요청하면 구체적인 최적화 방안을 제안받을 수 있습니다.
...
- 가상 스크롤(virtualization)로 대량 리스트 처리`,
        keyPoints: [
          '최적화는 측정 후에 진행 — 추측 기반 최적화는 효과가 제한적이다',
          'React.memo는 props가 자주 변하지 않는 컴포넌트에 효과적이다',
          'useMemo/useCallback을 모든 곳에 쓰면 오히려 메모리 낭비가 된다',
          'AI에게 Lighthouse 점수 개선 방안을 물으면 체계적인 가이드를 받을 수 있다'
        ],
        codeExamples: [
          {
            language: 'typescript',
            title: 'React 성능 최적화 예시',
            code: `// AI에게 요청: "이 컴포넌트의 리렌더링을 최적화해주세요"
...
function App() {
  ...
  // ─── 7. AI 디버깅 마스터 ───
  {
    ...
    slug: 'debugging',
    title: 'AI 디버깅 마스터',
    description: '에러 메시지 분석, 재현 단계 설명, 로그 기반 디버깅 등 AI와 함께 버그를 해결합니다.',
    level: 'intermediate',
    order: 7,
    contents: [
      {
        id: 'int-7-1',
        title: '에러 메시지 분석',
        description: '에러 메시지를 AI에게 효과적으로 전달하여 빠르게 원인을 파악하는 방법을 배웁니다.',
        content: `에러가 발생했을 때 AI에게 효과적으로 도움을 요청하려면, 에러의 맥락을 충분히 전달해야 합니다. 단순히 에러 메시지만 보내는 것보다 주변 상황을 함께 전달하면 훨씬 정확한 해결책을 얻을 수 있습니다.
...
- **네트워크 에러**: 요청/응답 정보와 API 엔드포인트를 공유`,
        keyPoints: [
          '에러 메시지를 일부만 발췌하면 AI가 잘못된 원인을 추측할 수 있다',
          '재현 단계를 포함하면 AI가 에러의 맥락을 정확히 파악한다',
          '최근 변경사항을 알려주면 원인 범위를 좁힐 수 있다',
          '스택 트레이스의 첫 번째 줄이 가장 중요한 단서이다'
        ],
        codeExamples: [
          {
            language: 'markdown',
            title: '효과적인 에러 보고 프롬프트',
            code: `## 에러 상황
...
(트리거 지연?)`
      ...
      },
      {
        id: 'int-7-2',
        title: '로그 기반 디버깅',
        description: '전략적인 로깅과 AI를 결합하여 복잡한 버그를 추적하는 방법을 배웁니다.',
        content: `복잡한 버그는 한 번에 원인을 찾기 어렵습니다. 전략적으로 로그를 추가하고, 그 결과를 AI에게 분석하게 하면 효율적으로 원인을 좁혀갈 수 있습니다.
...
- 에러 catch 블록`,
        keyPoints: [
          'console.log 대신 구조화된 로그(JSON 형태)를 사용하면 AI 분석이 용이하다',
          '로그에 타임스탬프를 포함하면 실행 순서를 파악할 수 있다',
          'AI에게 로그 전체를 전달하고 패턴 분석을 요청한다',
          '디버깅 완료 후 반드시 디버그 로그를 제거한다'
        ],
        codeExamples: [
          {
            language: 'typescript',
            title: '전략적 로그 추가 예시',
            code: `// AI에게 요청: "장바구니에 추가가 안 되는 버그가 있어요.
// 어디에 로그를 추가하면 좋을까요?"
...
// → setCart 부분에서 에러 발생 의심`
      ...
      },
      {
        id: 'int-7-3',
        title: '스택 트레이스 해석',
        description: 'JavaScript/TypeScript 스택 트레이스를 AI와 함께 분석하여 에러의 근본 원인을 찾습니다.',
        content: `스택 트레이스(Stack Trace)는 에러가 발생한 시점의 함수 호출 경로를 보여줍니다. 이를 올바르게 해석하면 에러의 근본 원인을 빠르게 찾을 수 있습니다.
...
- "이 스택 트레이스를 분석하고 원인을 설명해주세요"라고 요청합니다`,
        keyPoints: [
          '스택 트레이스의 맨 위 줄이 에러 발생 지점이다',
          '내 코드 파일명을 찾아 해당 줄을 먼저 확인한다',
          '라이브러리 내부 코드는 보통 무시하고 내 코드만 분석한다',
          '프로덕션에서는 소스맵이 없어 디버깅이 어려울 수 있다'
        ],
        codeExamples: [
          {
            language: 'markdown',
            title: '스택 트레이스 분석 프롬프트 예시',
            code: `## 에러 상황
...
4. 같은 유형의 에러를 방지하는 패턴`
      ...
      },
      {
        id: 'int-7-4',
        title: '수정 검증과 회귀 방지',
        description: '버그 수정 후 검증하고, 같은 버그가 재발하지 않도록 방지하는 전략을 배웁니다.',
        content: `버그를 수정한 후에는 반드시 검증 단계를 거쳐야 합니다. 또한 같은 유형의 버그가 다시 발생하지 않도록 예방 조치를 취해야 합니다.
...
AI에게 "이 수정이 다른 곳에 영향을 줄 수 있는지 분석해주세요"라고 요청하면 놓칠 수 있는 사이드 이펙트를 발견할 수 있습니다.`,
        keyPoints: [
          '버그 수정 후 원래 재현 단계로 반드시 검증한다',
          '수정이 다른 기능에 미치는 영향을 AI에게 분석 요청한다',
          '방어적 프로그래밍으로 같은 유형의 버그를 사전에 방지한다',
          'TypeScript strict mode는 컴파일 타임에 많은 버그를 잡아준다'
        ],
        codeExamples: [
          {
            language: 'typescript',
            title: '방어적 프로그래밍 패턴',
            code: `// ── 방어적 프로그래밍 Before / After ──
