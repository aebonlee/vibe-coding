    ...
    steps: [
      {
        step: 1,
        title: '프로젝트 설계',
        description: 'Todo 앱의 기능과 UI를 설계합니다.',
        content: `이 단계에서는 Lovable(lovable.dev)에 접속하여 새 프로젝트를 시작합니다. Todo 앱은 바이브 코딩의 첫 프로젝트로 가장 적합합니다. 작은 규모이면서도 CRUD(생성, 읽기, 수정, 삭제)의 모든 요소를 포함하고 있기 때문입니다. 먼저 어떤 기능이 필요한지 목록을 작성한 후, AI에게 구체적으로 설명하는 것이 핵심입니다.
...
**흔한 실수:**
프롬프트가 너무 짧으면 AI가 과도하게 복잡한 앱을 만들거나, 반대로 너무 단순한 앱을 만들 수 있습니다. "미니멀하게", "단순하게" 같은 키워드를 포함하면 초보자가 이해하기 쉬운 코드가 생성됩니다. 또한 컴포넌트 이름을 직접 지정하지 않으면 AI가 자체적으로 이름을 정하므로, 학습 자료와 일치하지 않을 수 있습니다.`,
        tips: [
          '프롬프트에 구체적인 기능을 번호로 나열하면 AI가 빠뜨리지 않습니다',
          'UI 스타일(미니멀, 모던, 컬러풀 등)을 함께 설명하면 디자인 품질이 올라갑니다',
          '컴포넌트 이름을 직접 지정하면 코드 구조를 이해하기 쉬워집니다'
        ]
      },
      {
        step: 2,
        title: 'UI 컴포넌트 생성',
        description: 'AI로 기본 UI를 생성합니다.',
        content: `생성된 코드를 꼼꼼히 확인하고 컴포넌트 구조를 이해하는 것이 이 단계의 핵심입니다. AI가 만들어준 코드를 그대로 사용하는 것이 아니라, 왜 이렇게 구조가 나뉘었는지를 파악해야 합니다. Header에는 입력 폼, TodoList에는 목록 렌더링, TodoItem에는 개별 항목의 체크/삭제 로직이 들어갑니다.
...
**흔한 실수:**
컴포넌트를 분리할 때 상태 관리 위치를 잘못 잡는 경우가 많습니다. todos 상태는 반드시 App(또는 공통 부모)에 있어야 하며, 자식 컴포넌트에서는 props로 받아 사용합니다. 이것이 React의 "상태 끌어올리기(Lifting State Up)" 패턴입니다.`,
        codeExamples: [
          {
            language: 'typescript',
            title: 'Todo 타입 정의',
  ...
  return (
    <div className="app">
      ...
      <TodoList todos={todos} onToggle={toggleTodo} onDelete={deleteTodo} />
        ...
        tips: [
          'AI가 생성한 코드의 import 경로가 맞는지 반드시 확인하세요',
          'Todo 타입에 id를 Date.now()로 만들면 간단하고 고유한 값이 됩니다',
          '컴포넌트 분리 후 화면이 깨지면 props 전달 누락을 먼저 확인하세요'
        ]
      },
      {
        step: 3,
        title: '기능 추가 - 필터링',
        description: '전체/완료/미완료 필터 기능을 추가합니다.',
        content: `필터링은 Todo 앱의 핵심 UX 기능입니다. 사용자가 "전체", "완료", "미완료" 버튼을 클릭하면 해당 조건에 맞는 항목만 표시됩니다. 이 기능을 구현하면서 배열의 filter 메서드와 조건부 렌더링을 자연스럽게 익힐 수 있습니다.
...
**흔한 실수:**
필터링 시 원본 todos 배열을 직접 수정하면 안 됩니다. setTodos로 필터링된 결과를 저장하면 원본 데이터가 사라져 "전체"로 돌아갈 수 없습니다. 반드시 useMemo나 별도 변수로 필터링된 결과를 계산하세요.`,
        codeExamples: [
          {
            language: 'typescript',
            title: '필터 상태 관리 및 적용',
            code: `type FilterType = 'all' | 'completed' | 'active';
...
const counts = useMemo(() => ({
  ...
  active: todos.filter(t => !t.completed).length,
          ...
          {
            language: 'tsx',
            title: 'TodoFilter 컴포넌트',
            code: `function TodoFilter({ filter, onFilterChange, counts }: TodoFilterProps) {
  ...
  return (
    <div className="filter-buttons">
      {filters.map(f => (
        ...
        tips: [
          'useMemo를 사용하면 todos가 변경될 때만 필터링이 재계산되어 성능이 좋아집니다',
          '필터 버튼에 항목 수를 표시하면 사용자 경험이 크게 향상됩니다',
          '필터링은 원본 배열을 변경하지 않고 새 배열을 반환하는 것이 핵심 원칙입니다'
        ]
      },
      {
        step: 4,
        title: 'localStorage 연동',
        description: '브라우저 새로고침 후에도 데이터가 유지되도록 합니다.',
        content: `현재까지 만든 Todo 앱은 새로고침하면 데이터가 모두 사라집니다. React의 useState는 메모리에만 데이터를 저장하기 때문입니다. localStorage를 사용하면 브라우저에 데이터를 영구적으로 저장할 수 있습니다. 이 단계에서는 useEffect를 활용하여 상태와 localStorage를 동기화하는 방법을 배웁니다.
...
**흔한 실수:**
useEffect의 의존성 배열을 빠뜨리면 무한 루프가 발생할 수 있습니다. 저장용 useEffect의 의존성 배열에는 [todos]를, 로드용 useEffect에는 []를 넣어야 합니다. 또한 초기 렌더링 시 빈 상태가 localStorage를 덮어쓰는 문제가 있으므로, useState의 초기값 함수(lazy initializer)를 사용하는 것이 더 안전합니다.`,
        codeExamples: [
          {
            language: 'typescript',
            title: 'localStorage 연동 (lazy initializer 패턴)',
            code: `const STORAGE_KEY = 'todos';
...
const [todos, setTodos] = useLocalStorage<Todo[]>('todos', []);`
        ...
        tips: [
          'useState의 lazy initializer(함수 전달)를 사용하면 초기 렌더링 시 localStorage를 한 번만 읽습니다',
          'JSON.parse는 항상 try-catch로 감싸야 합니다 - 손상된 데이터가 있으면 앱이 크래시됩니다',
          '개발자 도구(F12) > Application > Local Storage에서 저장된 데이터를 직접 확인하고 디버깅하세요'
        ]
      },
      {
        step: 5,
        title: '스타일링 및 완성',
        description: '디자인을 다듬고 반응형으로 만듭니다.',
        content: `마지막 단계에서는 Todo 앱의 시각적 완성도를 높입니다. 기능이 완벽하더라도 디자인이 거칠면 사용하기 불편합니다. 반응형 디자인, 애니메이션, 접근성을 모두 챙기면 프로페셔널한 결과물이 됩니다.
...
**흔한 실수:**
CSS 애니메이션을 항목 삭제에 적용하려면 DOM에서 즉시 제거하면 안 됩니다. 삭제 애니메이션이 끝난 후 제거해야 하므로, onAnimationEnd 이벤트나 setTimeout을 활용해야 합니다. 처음에는 간단한 opacity 전환만 적용하고, 익숙해지면 height 애니메이션을 추가하세요.`,
        codeExamples: [
          {
            language: 'css',
            title: '핵심 스타일링',
...
@media (max-width: 480px) {
  ...
  .todo-item button { min-height: 44px; min-width: 44px; }
        ...
        tips: [
          '모바일 터치 영역은 최소 44x44px이어야 합니다 (Apple HIG 기준)',
          'transition은 hover 효과에, animation은 등장/퇴장 효과에 사용하세요',
          '빈 상태(Empty State) 메시지를 추가하면 UX가 크게 향상됩니다'
        ]
      }
    ]
  },
  {
    ...
    diagram: `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 800 460" font-family="'Noto Sans KR', sans-serif">
  <defs>
    <marker id="port-arrow" markerWidth="10" markerHeight="7" refX="10" refY="3.5" orient="auto">
      <polygon points="0 0, 10 3.5, 0 7" fill="#0046C8"/>
    </marker>
  </defs>
  <rect x="1" y="1" width="798" height="458" rx="12" fill="#FFFFFF" stroke="#E5E7EB" stroke-width="2"/>
  <text x="400" y="32" text-anchor="middle" font-size="16" font-weight="bold" fill="#111827">포트폴리오 섹션 레이아웃 &amp; 반응형 구조</text>
  <!-- 섹션 흐름 (세로) -->
  <text x="190" y="62" text-anchor="middle" font-size="12" font-weight="bold" fill="#4B5563">페이지 섹션 흐름</text>
  <rect x="100" y="76" width="180" height="42" rx="8" fill="#F0F4FF" stroke="#0046C8" stroke-width="2"/>
  <text x="190" y="103" text-anchor="middle" font-size="14" fill="#111827">Hero (메인 비주얼)</text>
  <line x1="190" y1="118" x2="190" y2="140" stroke="#0046C8" stroke-width="2" marker-end="url(#port-arrow)"/>
  <rect x="100" y="145" width="180" height="42" rx="8" fill="#F0F4FF" stroke="#0046C8" stroke-width="2"/>
  <text x="190" y="172" text-anchor="middle" font-size="14" fill="#111827">About (자기소개)</text>
  <line x1="190" y1="187" x2="190" y2="209" stroke="#0046C8" stroke-width="2" marker-end="url(#port-arrow)"/>
  <rect x="100" y="214" width="180" height="42" rx="8" fill="#F0F4FF" stroke="#0046C8" stroke-width="2"/>
  <text x="190" y="241" text-anchor="middle" font-size="14" fill="#111827">Skills (기술 스택)</text>
  <line x1="190" y1="256" x2="190" y2="278" stroke="#0046C8" stroke-width="2" marker-end="url(#port-arrow)"/>
  <rect x="100" y="283" width="180" height="42" rx="8" fill="#F0F4FF" stroke="#0046C8" stroke-width="2"/>
  <text x="190" y="310" text-anchor="middle" font-size="14" fill="#111827">Projects (프로젝트)</text>
  <line x1="190" y1="325" x2="190" y2="347" stroke="#0046C8" stroke-width="2" marker-end="url(#port-arrow)"/>
  <rect x="100" y="352" width="180" height="42" rx="8" fill="#F0F4FF" stroke="#0046C8" stroke-width="2"/>
  <text x="190" y="379" text-anchor="middle" font-size="14" fill="#111827">Contact (연락처)</text>
  <!-- 반응형 브레이크포인트 -->
  <text x="560" y="62" text-anchor="middle" font-size="12" font-weight="bold" fill="#4B5563">반응형 브레이크포인트</text>
  <!-- 데스크톱 -->
  <rect x="435" y="76" width="250" height="65" rx="8" fill="#F0F4FF" stroke="#0046C8" stroke-width="2"/>
  <text x="560" y="100" text-anchor="middle" font-size="13" font-weight="bold" fill="#111827">데스크톱 (1024px+)</text>
  <text x="560" y="126" text-anchor="middle" font-size="12" fill="#4B5563">2열 그리드 / 내비게이션 바</text>
  <line x1="560" y1="141" x2="560" y2="165" stroke="#0046C8" stroke-width="2" marker-end="url(#port-arrow)"/>
  <!-- 태블릿 -->
  <rect x="435" y="170" width="250" height="65" rx="8" fill="#F0F4FF" stroke="#0046C8" stroke-width="2"/>
  <text x="560" y="194" text-anchor="middle" font-size="13" font-weight="bold" fill="#111827">태블릿 (768px~1023px)</text>
  <text x="560" y="220" text-anchor="middle" font-size="12" fill="#4B5563">1~2열 그리드 / 햄버거 메뉴</text>
  <line x1="560" y1="235" x2="560" y2="259" stroke="#0046C8" stroke-width="2" marker-end="url(#port-arrow)"/>
  <!-- 모바일 -->
  <rect x="435" y="264" width="250" height="65" rx="8" fill="#F0F4FF" stroke="#0046C8" stroke-width="2"/>
  <text x="560" y="288" text-anchor="middle" font-size="13" font-weight="bold" fill="#111827">모바일 (&lt; 768px)</text>
  <text x="560" y="314" text-anchor="middle" font-size="12" fill="#4B5563">1열 스택 / 풀 와이드</text>
  <!-- 다크모드 토글 -->
  <line x1="560" y1="329" x2="560" y2="358" stroke="#0046C8" stroke-width="2" marker-end="url(#port-arrow)"/>
  <rect x="435" y="363" width="250" height="42" rx="8" fill="#0046C8" stroke="#0046C8" stroke-width="2"/>
  <text x="560" y="390" text-anchor="middle" font-size="14" font-weight="bold" fill="#fff">다크모드 토글 (CSS 변수)</text>
</svg>`,
    steps: [
      {
        step: 1,
        title: '구조 설계',
        description: '포트폴리오의 섹션을 기획합니다.',
        content: `포트폴리오 사이트는 자신을 효과적으로 소개하는 웹사이트입니다. 이 프로젝트에서는 v0.dev(vercel.com/v0)를 사용하여 섹션별로 디자인을 생성합니다. v0은 UI 디자인에 특화된 AI 도구로, 특히 모던하고 깔끔한 컴포넌트를 잘 만들어줍니다.
...
**흔한 실수:**
처음부터 모든 섹션을 한 번에 만들려고 하면 프롬프트가 너무 길어져 AI가 일부를 빠뜨릴 수 있습니다. 먼저 전체 구조를 잡은 후, 섹션별로 디테일을 추가하는 것이 효과적입니다.`,
        tips: [
          'v0.dev에서 생성한 코드는 shadcn/ui 컴포넌트를 사용합니다 - 필요한 패키지 설치를 확인하세요',
          '한 번에 완벽한 결과를 기대하지 말고, 섹션별로 나눠서 프롬프트하세요',
          '레퍼런스 사이트 URL을 프롬프트에 포함하면 스타일 방향을 잡는 데 도움이 됩니다'
        ]
      },
      {
        step: 2,
        title: 'Hero 섹션',
        description: '메인 비주얼과 소개 영역을 만듭니다.',
        content: `Hero 섹션은 방문자가 가장 먼저 보는 영역입니다. 첫인상을 결정하므로 깔끔하고 임팩트 있게 만들어야 합니다. 이름, 직함, 간단한 소개 문구, 그리고 소셜 링크 버튼이 핵심 요소입니다.
...
**흔한 실수:**
풀스크린 높이를 100vh로 설정할 때 모바일에서 주소창 때문에 콘텐츠가 잘릴 수 있습니다. dvh(dynamic viewport height) 단위를 사용하거나, min-height: 100vh로 설정하면 이 문제를 해결할 수 있습니다.`,
        codeExamples: [
          {
            language: 'tsx',
            title: 'Hero 섹션 기본 구조',
  ...
  return (
    <section id="hero" className="hero-section">
      ...
      <div className="scroll-indicator">
        ...
        tips: [
          'min-height: 100dvh를 사용하면 모바일 주소창 문제를 해결할 수 있습니다',
          'Hero 섹션의 텍스트는 3초 내에 핵심을 전달할 수 있도록 간결하게 작성하세요',
          'CTA 버튼은 최대 2개까지만 배치하세요 - 너무 많으면 선택 장애가 생깁니다'
        ]
      },
      {
        step: 3,
        title: 'About & Skills',
        description: '자기소개와 기술 스택을 표시합니다.',
        content: `About 섹션은 자신을 소개하는 영역이고, Skills 섹션은 보유한 기술 스택을 시각적으로 보여주는 영역입니다. 이 두 섹션은 채용 담당자가 가장 주의 깊게 보는 부분이므로, 정보의 가독성과 시각적 매력을 모두 잡아야 합니다.
...
**흔한 실수:**
프로필 이미지를 너무 크게 설정하면 모바일에서 화면 대부분을 차지합니다. max-width를 200~250px로 제한하세요. Skills에서 프로그레스 바(실력 퍼센트)를 사용하는 것은 논란이 있습니다. 객관적 기준이 없어 오히려 마이너스가 될 수 있으므로, 태그 형태를 추천합니다.`,
        codeExamples: [
          {
            language: 'tsx',
            title: 'Skills 섹션 - 카테고리별 태그',
  ...
  return (
    <section id="skills" className="skills-section">
      ...
      <div className="skills-grid">
        ...
        tips: [
          '프로필 이미지는 정사각형 비율로 준비하고, CSS로 원형(border-radius: 50%)으로 만드세요',
          'Skills에서 프로그레스 바 대신 태그 형태를 추천합니다 - 실력 퍼센트는 객관성이 부족합니다',
          'Intersection Observer API를 사용하면 스크롤 시 섹션이 나타나는 애니메이션을 쉽게 구현할 수 있습니다'
        ]
      },
      {
        step: 4,
        title: 'Projects 섹션',
        description: '프로젝트 카드 그리드를 만듭니다.',
        content: `Projects 섹션은 포트폴리오의 핵심입니다. 자신이 만든 프로젝트를 카드 형태로 보여주며, 각 카드에는 프로젝트 제목, 설명, 사용 기술, 라이브 링크, GitHub 링크가 포함됩니다. 시각적으로 매력적인 카드 디자인이 중요합니다.
...
**흔한 실수:**
이미지 비율이 카드마다 다르면 그리드가 들쭉날쭉해집니다. object-fit: cover와 고정 높이를 사용하여 모든 카드의 이미지 영역을 통일하세요. 설명이 길어지면 카드 높이가 달라지므로 line-clamp로 줄 수를 제한하세요.`,
        codeExamples: [
          {
            language: 'typescript',
            title: '프로젝트 데이터 구조',
...
.card-description {
  ...
  overflow: hidden;
        ...
        tips: [
          'object-fit: cover로 이미지 비율을 통일하면 그리드가 깔끔해집니다',
          'line-clamp로 설명 텍스트 줄 수를 제한하면 카드 높이가 일정해집니다',
          '프로젝트 스크린샷은 직접 캡처하거나, placeholder 서비스(placehold.co)를 임시로 사용하세요'
        ]
      },
      {
        step: 5,
        title: '다크모드',
        description: '라이트/다크 테마 토글을 구현합니다.',
        content: `다크모드는 현대 웹사이트의 필수 기능입니다. CSS 변수(Custom Properties)를 활용하면 테마 전환을 깔끔하게 구현할 수 있습니다. body 또는 html에 data-theme 속성을 추가하고, CSS 변수 값을 바꾸는 방식입니다.
...
**흔한 실수:**
CSS 변수를 일부 요소에만 적용하면 다크모드에서 특정 영역만 밝게 남는 문제가 발생합니다. 모든 배경색, 텍스트 색, 보더 색을 CSS 변수로 관리해야 합니다. 또한 이미지(로고, 프로필)가 다크모드에서 잘 보이는지도 확인해야 합니다.`,
        codeExamples: [
          {
            language: 'css',
            title: 'CSS 변수 기반 테마 시스템',
            code: `:root {
  --bg-primary: #ffffff;
  --bg-secondary: #f9fafb;
  --text-primary: #111827;
...
body {
  ...
  return { isDark, toggle };
        ...
        tips: [
          '모든 색상을 CSS 변수로 관리해야 다크모드 전환 시 빠뜨리는 곳이 없습니다',
          'prefers-color-scheme 미디어 쿼리로 시스템 설정을 감지하면 첫 방문자의 UX가 좋아집니다',
          'transition을 body에 적용할 때 duration은 0.3s가 자연스럽습니다 - 너무 길면 답답합니다'
        ]
      },
      {
        step: 6,
        title: '반응형 및 배포',
        description: '모바일 대응과 배포를 진행합니다.',
        content: `마지막 단계에서는 모든 섹션의 반응형 레이아웃을 점검하고, 실제 배포하여 온라인에 공개합니다. 반응형 디자인은 데스크톱, 태블릿, 모바일 3가지 화면 크기에서 모두 잘 보이도록 만드는 것입니다.
...
**흔한 실수:**
반응형 테스트를 크롬 개발자 도구의 "모바일 시뮬레이터"로만 하면 실제 기기에서 차이가 날 수 있습니다. 가능하면 실제 스마트폰에서도 확인하세요. 특히 iOS Safari에서 100vh 문제, 폰트 렌더링 차이, 터치 이벤트 등이 다를 수 있습니다.`,
        codeExamples: [
          {
            language: 'css',
            title: '반응형 미디어 쿼리 구조',
...
@media (max-width: 480px) {
  ...
  .hero-name { font-size: 1.5rem; }
        ...
        tips: [
          '개발자 도구(F12)에서 다양한 기기 크기로 테스트하되, 실제 기기에서도 반드시 확인하세요',
          'GitHub Pages 배포 시 vite.config.ts의 base 경로를 확인하세요',
          '네비게이션 smooth scroll은 CSS의 scroll-behavior: smooth 한 줄로 구현할 수 있습니다'
        ]
      }
    ]
  },
    ...
    steps: [
      {
        step: 1,
        title: '랜딩 기획',
        description: 'SaaS 제품의 컨셉과 섹션을 기획합니다.',
        content: `SaaS 랜딩 페이지는 방문자를 유료 고객으로 전환시키는 것이 목표입니다. 따라서 "전환 퍼널(Conversion Funnel)" 개념을 이해하는 것이 중요합니다. 방문자가 페이지를 스크롤하면서 제품의 가치를 이해하고, 최종적으로 구독이나 무료 체험을 시작하도록 유도해야 합니다.
...
**흔한 실수:**
SaaS 랜딩의 가장 큰 실수는 기능 나열에만 집중하는 것입니다. 방문자는 기능보다 "이 제품이 나의 문제를 어떻게 해결해주는가"에 관심이 있습니다. 헤드라인에 기능이 아닌 "결과(benefit)"를 강조하세요.`,
        tips: [
          'SaaS 랜딩의 핵심은 "기능"이 아니라 "고객의 문제 해결"입니다 - 헤드라인에 benefit을 강조하세요',
          'Bolt.new는 전체 프로젝트를 생성하므로, 한 번에 모든 섹션을 프롬프트해도 잘 동작합니다',
          '실제 SaaS 사이트(Notion, Slack, Linear 등)를 참고하면 디자인 감각을 빠르게 키울 수 있습니다'
        ]
      },
      {
        step: 2,
        title: 'Hero & Features',
        description: '메인 비주얼과 기능 소개 영역을 만듭니다.',
        content: `Hero 섹션은 랜딩 페이지에서 가장 중요한 영역입니다. 방문자의 78%가 Hero 섹션만 보고 이탈하거나 계속 스크롤할지를 결정합니다. 따라서 강력한 헤드라인, 명확한 부제목, 시각적인 제품 이미지가 필수입니다.
...
**흔한 실수:**
CTA 버튼 텍스트가 너무 일반적이면(예: "자세히 알아보기") 클릭률이 낮아집니다. "무료로 시작하기", "14일 무료 체험" 같은 행동 지향적이고 구체적인 텍스트를 사용하세요. Hero 이미지가 없으면 텍스트만으로는 시각적 임팩트가 부족합니다.`,
        codeExamples: [
          {
            language: 'tsx',
            title: 'Hero 섹션 구조',
  ...
  return (
    <section className="py-20 bg-white">
      <div className="container mx-auto px-4">
        ...
        tips: [
          'Hero 헤드라인은 "무엇을(What)"이 아닌 "왜(Why)" 또는 "결과(Result)"에 초점을 맞추세요',
          'CTA 버튼은 대비가 강한 색상으로, 페이지에서 가장 눈에 띄어야 합니다',
          'Features는 3-4개가 적당합니다 - 너무 많으면 오히려 핵심이 흐려집니다'
        ]
      },
      {
        step: 3,
        title: '가격 테이블',
        description: '3단계 가격 플랜을 만듭니다.',
        content: `가격 테이블(Pricing Table)은 SaaS 랜딩에서 전환율에 가장 직접적인 영향을 미치는 섹션입니다. 보통 3단계(Free, Pro, Enterprise) 구조를 사용하며, 가운데 플랜을 "추천"으로 강조하여 시선을 유도하는 것이 일반적입니다. 이를 "앵커링 효과"라고 합니다.
...
**흔한 실수:**
가격 카드 간 기능 비교가 어려우면 사용자가 혼란을 느낍니다. 모든 플랜에서 동일한 순서로 기능을 나열하고, 미포함 기능은 취소선이나 흐린 색으로 표시하세요. 체크마크와 엑스 아이콘을 사용하면 한눈에 비교할 수 있습니다.`,
        codeExamples: [
          {
            language: 'typescript',
            title: '가격 데이터 구조',
...
<div className="flex items-center justify-center gap-3 mb-12">
  ...
  <span className={isYearly ? 'font-bold' : 'text-gray-500'}>
    연간 <span className="text-green-500 text-sm">20% 할인</span>
        ...
        tips: [
          '가운데 플랜(Pro)을 시각적으로 강조하면 자연스럽게 선택이 유도됩니다 (앵커링 효과)',
          '연간 결제 할인율을 명확하게 표시하면 연간 결제 전환율이 높아집니다',
          'Enterprise는 고정 가격 대신 "문의하기"를 사용하여 영업팀 연결 기회를 만드세요'
        ]
      },
      {
        step: 4,
        title: '구독 폼 & CTA',
        description: '이메일 구독 폼과 액션 버튼을 추가합니다.',
        content: `CTA(Call to Action)와 구독 폼은 랜딩 페이지의 최종 전환 요소입니다. 방문자가 스크롤을 끝까지 내렸다는 것은 제품에 관심이 있다는 의미이므로, 마지막으로 강력한 행동 유도를 해야 합니다. 뉴스레터 구독 폼, 무료 체험 시작 버튼, 데모 예약 폼 등 다양한 형태가 가능합니다.
...
**흔한 실수:**
폼 유효성 검증을 빠뜨리면 빈 이메일이나 잘못된 형식이 제출됩니다. 이메일 형식 검증, 빈 값 체크, 제출 중 로딩 상태, 성공/실패 피드백을 반드시 구현하세요. 또한 CTA 문구에 "무료"와 "카드 불필요"를 명시하면 전환율이 크게 올라갑니다.`,
        codeExamples: [
          {
            language: 'tsx',
            title: 'CTA 섹션 + 이메일 폼',
  ...
  return (
    <div className="bg-gray-50 p-6 rounded-xl shadow-sm">
      ...
      <div className="flex items-center gap-3">
        ...
        tips: [
          'CTA 문구에 "무료"와 "카드 불필요"를 명시하면 전환율이 평균 30% 높아집니다',
          '폼 제출 후 성공/실패 피드백을 반드시 보여주세요 - 사용자는 자신의 행동 결과를 확인하고 싶어합니다',
          'Testimonial은 실제 사진과 구체적인 직함이 있을수록 신뢰도가 높아집니다'
        ]
      },
      {
        step: 5,
        title: '반응형 완성',
        description: '모바일에서도 완벽하게 동작하도록 합니다.',
        content: `SaaS 랜딩 페이지는 모바일 트래픽이 50% 이상을 차지합니다. 특히 소셜 미디어(Instagram, Twitter)에서 유입되는 경우 거의 100% 모바일입니다. 따라서 모바일에서의 경험이 전환율에 직접적인 영향을 미칩니다.
...
**흔한 실수:**
Tailwind CSS의 반응형 접두사(sm:, md:, lg:)를 잘못 사용하면 예상과 다른 레이아웃이 나옵니다. Tailwind는 모바일 퍼스트이므로, 기본 스타일이 모바일이고 md:, lg:로 확장하는 구조입니다. 예를 들어 "grid-cols-1 md:grid-cols-2 lg:grid-cols-4"는 모바일 1열, 태블릿 2열, 데스크톱 4열을 의미합니다.`,
        codeExamples: [
          {
            language: 'tsx',
            title: 'Tailwind 반응형 레이아웃 패턴',
            code: `{/* Features: 모바일 1열 → 태블릿 2열 → 데스크톱 4열 */}
...
<img src="/mockup.webp" alt="제품 화면" loading="lazy"
  className="w-full rounded-xl shadow-lg" />`
        ...
        tips: [
          'Tailwind는 모바일 퍼스트입니다 - 기본이 모바일이고 md:/lg:로 확장하는 구조를 기억하세요',
          'Lighthouse(개발자 도구 > Lighthouse)로 성능, 접근성, SEO 점수를 측정하세요 - 90점 이상이 목표',
          'CSS order 속성으로 모바일에서 중요한 요소(추천 플랜)를 먼저 보여줄 수 있습니다'
        ]
      }
    ]
  }
